...
function hasAudio(filePath) {
  try {
    ...
    console.error(`Failed to probe audio for ${filePath}:`, err);
    return false;
  }
}
...
async function processVideoJob(job) {
  ...
  let logoStreamIdx = -1;
  if (hasLogo) {
    step1InputsRaw.push(`-loop 1 ${loopLimitFlag} -i "${logoPath}"`);
  ...
  const baseHasAudio = hasAudio(baseVideoPath);
  const audioMapping = (job.strip_original_audio === false && baseHasAudio)
  ...
  let step2Cmd = "";
  const mainHasAudio = hasAudio(layeredVideoPath);
  ...
  if (audioUrl) {
  ...
  } else if (job.strip_original_audio === false && mainHasAudio) {
    // Keep original audio of base video: concat video streams + map main audio with fade out
    if (hasOutro) {
      step2Cmd = [
        `ffmpeg -y`,
        `-i "${layeredVideoPath}"`,
        ...
        `-filter_complex "${filterComplexString}"`,
        `-map "[outv]"`,
        `-map 0:a`,               // Map audio from layered_main.mp4
        `-filter:a "afade=t=out:st=${fadeOutStart}:d=0.5"`,
        `-c:v libx264 -preset fast -pix_fmt yuv420p`,
        `-c:a aac -b:a 192k`,
        `"${finalOutputPath}"`
    ...
    // No music and strip original audio (or main has no audio)
    if (hasOutro) {
      step2Cmd = [
        `ffmpeg -y`,
        `-i "${layeredVideoPath}"`,
